...
export default function ScopeSetPanel({ title, listPath, addPath, delPath, icon: Icon, keyFormat = "none", supportsEntity = true, transport = "query" }: any) {
  ...
  const load = async () => {
    ...
    try {
      if (keyFormat === "suffix" || keyFormat === "prefix" || keyFormat === "domain") {
        const arr = await http<string[]>(listPath);
        setRows((Array.isArray(arr) ? arr : []).map(parseKey));
      } else {
        const data = await http<Item[]>(listPath);
        setRows(data);
      }
    } catch {
      setRows([]);
  ...
  const add = async () => {
    ...
    if (transport === "query") {
      ...
      await fetch(`${addPath}?${qs.toString()}`, { method: "POST" });
    } else {
      await fetch(addPath, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ domain, entity: entity ?? null }) });
    }
    setDomain(""); setEntity(undefined);
    await load();
  };
  ...
  const remove = async (d: string, e?: string | null) => {
    ...
    await fetch(`${delPath}?${qs.toString()}`, { method: "DELETE" });
    await load();
  };
    ...
    <Card className="shadow-sm">
      <CardContent className="p-4">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 rounded-2xl shadow-sm border"><Icon className="w-5 h-5" /></div>
          <div>
            <h3 className="text-lg font-semibold">{title}</h3>
            <p className="text-xs text-muted-foreground">Scope optionally narrowed by entity</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div className="grid gap-1">
              ...
              <Input placeholder="example.gov" value={domain} onChange={(e) => setDomain(e.target.value)} />
            </div>
            {supportsEntity && (
              <div className="grid gap-1">
                <Label className="text-xs text-muted-foreground">Entity (optional)</Label>
                <Select value={entity ?? undefined} onValueChange={(v) => setEntity(v === "__ANY__" ? undefined : (v as CrawledEntity))}>
                  <SelectTrigger className="w-[180px]"><SelectValue placeholder="Any" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="__ANY__">Any</SelectItem>
                    {ENTITY_OPTIONS.map((v) => (<SelectItem key={v} value={v}>{v}</SelectItem>))}
                  </SelectContent>
                </Select>
              </div>
            )}
          ...
          <div className="grid gap-2">
            {rows.map((r, idx) => (
              <div key={idx} className="flex items-center justify-between border rounded-xl px-3 py-2">
                <div className="text-sm"><span className="font-medium">{r.domain}</span>{" "}<span className="text-muted-foreground">{r.entity || (supportsEntity ? "Any" : "")}</span></div>
                <Button variant="ghost" size="icon" onClick={() => remove(r.domain, r.entity || undefined)}><Trash2 className="w-4 h-4" /></Button>
              </div>
            ))}
          </div>
